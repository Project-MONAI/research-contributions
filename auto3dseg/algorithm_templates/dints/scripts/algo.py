...
class DintsAlgo(BundleAlgo):
    def fill_template_config(self, data_stats_file, output_path, **kwargs):
        ...
        if kwargs.pop("fill_with_datastats", True):
            ...
            if ("class_names" in data_src_cfg and
                isinstance(data_src_cfg["class_names"], list) and
                "index" in data_src_cfg["class_names"][0]):

                class_index = [x["index"] for x in data_src_cfg["class_names"]]

                pt_type_transform_train = {"_target_": "CastToTyped",
                    "keys": ["@image_key", "@label_key"],
                    "dtype": ["$torch.float32", "$torch.float32"]}

                pt_type_transform_valid = {"_target_": "CastToTyped",
                    "keys": ["@image_key", "@label_key"],
                ...
                if ("sigmoid" in data_src_cfg and
                    isinstance(data_src_cfg["sigmoid"], bool) and
                    data_src_cfg["sigmoid"]):
                    hyper_parameters.update(
                        {"training#output_classes": len(data_src_cfg["class_names"])})
                    hyper_parameters_search.update(
                        {"searching#output_classes": len(data_src_cfg["class_names"])})
