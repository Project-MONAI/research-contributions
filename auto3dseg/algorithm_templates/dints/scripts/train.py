...
def run(config_file: Optional[Union[str, Sequence[str]]] = None, **override):
    ...
    if valid_at_orig_resolution_at_last or valid_at_orig_resolution_only:
        ...
        if "class_names" in parser and isinstance(
                parser["class_names"],
                list) and "index" in parser["class_names"][0]:
    ...
    try:
        ...
        if isinstance(class_names[0], dict):
            class_names = [class_names[_i]["name"]
    ...
    train_cache_rate = float(
        parser.get_parsed_content("training#train_cache_rate"))
    ...
    if not valid_at_orig_resolution_only:
        ...
        val_loader = DataLoader(val_ds, num_workers=parser.get_parsed_content(
            "training#num_workers_validation"), batch_size=1, shuffle=False, )
    ...
    if softmax:
        post_pred = transforms.Compose(
            [transforms.EnsureType(), transforms.AsDiscrete(argmax=True, to_onehot=None)])
    else:
        post_pred = transforms.Compose(
            [
    ...
    if parser["finetune"]["activate_finetune"] and os.path.isfile(
            os.path.join(ckpt_path, "progress.yaml")):
    ...
    if torch.cuda.device_count() == 1 or dist.get_rank() == 0:
        ...
        if es:
            ...
            early_stopping = EarlyStopping(
                patience=es_patience, delta=es_delta, verbose=True)
        ...
        if not valid_at_orig_resolution_only:
            ...
            for _round in range(num_rounds) if torch.cuda.device_count(
                ...
                for batch_data in train_loader:
                    ...
                    inputs_l = (
                        batch_data["image"].as_tensor() if isinstance(
                            batch_data["image"],
                            monai.data.MetaTensor) else batch_data["image"])
                    labels_l = (
                        batch_data["label"].as_tensor() if isinstance(
                            batch_data["label"],
                            monai.data.MetaTensor) else batch_data["label"])
                ...
                if torch.cuda.device_count() > 1:
                    dist.all_reduce(
                        loss_torch, op=torch.distributed.ReduceOp.SUM)
                ...
                with torch.no_grad():
                    ...
                    for val_data in val_loader:
                        ...
                        logger.debug(
                            f"{_index + 1} / {len(val_loader)}: {value}")
                    ...
                    if torch.cuda.device_count() > 1:
                        dist.all_reduce(
                            metric, op=torch.distributed.ReduceOp.SUM)
                    ...
                    if torch.cuda.device_count() == 1 or dist.get_rank() == 0:
                        for _c in range(metric_dim):
                            ...
                            try:
                                ...
                                writer.add_scalar(
                                    f"val_class/acc_{_c}", metric[2 * _c] / metric[2 * _c + 1], epoch)
                        ...
                        if avg_metric > best_metric:
                            ...
                            dict_file = {}
                            dict_file["best_avg_dice_score"] = float(
                                best_metric)
    ...
    if torch.cuda.device_count() == 1 or dist.get_rank() == 0:
        if (not valid_at_orig_resolution_only) and es and (
                _round + 1) < num_rounds:
            logger.warning(
