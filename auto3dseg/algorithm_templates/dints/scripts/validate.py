...
def run(config_file: Optional[Union[str, Sequence[str]]] = None, **override):
    ...
    if (
        "class_names" in parser
        and isinstance(parser["class_names"], list)
        and "index" in parser["class_names"][0]
    ):
    ...
    with torch.no_grad():
        ...
        for val_data in val_loader:
            ...
            val_data = [post_transforms(i)
                        for i in monai.data.decollate_batch(val_data)]
            ...
            if softmax:
                ...
                for _k in range(1, metric_dim + 1):
                    value[0,
                          _k - 1] = compute_dice(y_pred=(val_outputs == _k).float(),
