...
config_file = []
config_file.append(
    os.path.join("work_dir", "segresnet2d_0", "configs", "hyper_parameters.yaml")
)
config_file.append(os.path.join("work_dir", "segresnet2d_0", "configs", "network.yaml"))
config_file.append(
    os.path.join("work_dir", "segresnet2d_0", "configs", "transforms_train.yaml")
)
config_file.append(
    os.path.join("work_dir", "segresnet2d_0", "configs", "transforms_validate.yaml")
)
...
for _k in range(len(data_stat["stats_by_cases"])):
    ...
    for _l in range(3):
        if _l < 2:
            ...
            max_shape[_l] = max(max_shape[_l], int(image_shape[_l]))
    ...
    with torch.no_grad():
        for _k in range(num_iterations_validation):
            ...
            val_outputs = torch.zeros(
                (1, output_classes, max_shape[0], max_shape[1], max_shape[2])
            )
