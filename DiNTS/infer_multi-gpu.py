...
def main():
    ...
    infer_files = files
    infer_files = partition_dataset(data=infer_files, shuffle=False, num_partitions=dist.get_world_size(), even_divisible=False)[dist.get_rank()]
    ...
    infer_transforms = creating_transforms_testing(foreground_crop_margin, intensity_range, intensity_norm_transforms, spacing)
    ...
    with torch.no_grad():
        ...
        for infer_data in infer_loader:
            ...
            out_filename = os.path.join(args.output_root, infer_data["image_meta_dict"]["filename_or_obj"][0].split(os.sep)[-1])
            # out_filename = out_filename.replace("case_", "prediction_") + ".nii.gz"
            # out_affine = infer_data["image_meta_dict"]["original_affine"].numpy().squeeze()
            out_img = nib.Nifti1Image(out_nda.astype(np.uint8), None)
            ...
            if args.prob:
                for _k in range(1, output_classes):
                    ...
                    out_img = nib.Nifti1Image(infer_outputs_indiv, None)
