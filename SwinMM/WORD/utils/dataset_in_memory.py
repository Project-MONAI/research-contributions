...
from typing import MutableMapping, Union, Optional, Sequence, Callable
...
def is_deterministic_transform(transform) -> bool:
    return not (isinstance(transform, monai_transforms.Randomizable) or
                not isinstance(transform, monai_transforms.Transform))
...
class CachedDataset(torch_dataset.Dataset):
    ...
    def __init__(self,
                 data: Sequence,
                 transform: Optional[Union[Sequence[Callable],
                                           Callable]] = None,
        ...
        if hosts is None:
            raise ValueError(
                "We don't init bagua, have to manually launch redis")
    ...
    def _apply_deterministic_transform(self, item):
        ...
        if self.as_contiguous:
            item = monai_transforms.convert_to_contiguous(
