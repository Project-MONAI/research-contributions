...
def get_dataset_kwargs(dataset_name: str, stage: str, use_normal_dataset: bool,
...
def create_dataloader(
        data_files: Sequence[Any], is_testing: bool, transform: Callable, num_workers: int,
        is_distributed: bool, with_cache: bool, use_multi_epochs_loader: bool,
        batch_size: int,
        dataset_kwargs: MutableMapping[str,
                                       Any]) -> torch.utils.data.DataLoader:
    if not with_cache:
        dataset = data.Dataset(data=data_files,
                               transform=transform,
                               **dataset_kwargs)
    else:
        dataset = dataset_in_memory.CachedDataset(data=data_files,
                                                  transform=transform,
                                                  **dataset_kwargs)
    sampler = torch.utils.data.DistributedSampler(
        dataset, shuffle=not is_testing) if is_distributed else None
...
def get_loader(args):
    ...
    def _get_subset_loader(subset_name: str, transform: Callable,
                           is_testing: bool, use_normal_dataset: bool):
        datalist = load_decathlon_datalist(datalist_json,
                                           True,
                                           subset_name,
                                           base_dir=data_dir)
        dataset_kwargs = get_dataset_kwargs(subset_name, 'finetune',
                                            use_normal_dataset, args)
        loader = create_dataloader(datalist, is_testing, transform,
