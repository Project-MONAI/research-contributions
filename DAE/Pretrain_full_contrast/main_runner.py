...
from utils import load_checkpoint, save_checkpoint, get_grad_norm, auto_resume_helper, reduce_tensor,TensorboardLogger
import pdb
import torchvision
...
def main(args):
    ...
    for epoch in range(args.start_epoch, args.epoch):
        ...
        train_loss_avg, train_L1_avg, train_MM_avg = train_one_epoch(args, model, data_loader_train, optimizer, epoch, lr_scheduler)
        ...
        if dist.get_rank() == 0:
            ...
            if val_loss_avg <= val_loss_best:
                save_checkpoint(args, epoch, model_without_ddp, 0., optimizer, lr_scheduler, logger, best_model=True)

        if dist.get_rank() == 0 and (epoch == (args.epoch - 1)):
            save_checkpoint(args, epoch, model_without_ddp, 0., optimizer, lr_scheduler, logger, best_model=False)
...
def train_one_epoch(args, model, data_loader, optimizer, epoch, lr_scheduler):
    ...
    for idx, (img, mask) in enumerate(data_loader):
        ...
        if args.choice=="mae":
            ...
            img_lowres = F.interpolate(img,size=(int(96/args.interpolate),int(96/args.interpolate),int(96/args.interpolate)))
            ...
            img_lowres = F.interpolate(img_noisy,size=(int(96/args.interpolate),int(96/args.interpolate),int(96/args.interpolate)))
            img_resam = F.interpolate(img_lowres,size=(96,96,96))
            loss1,loss2, _, _ = model(img_resam, mask, img,cl_type)
        mask = mask.cuda(non_blocking=True)
