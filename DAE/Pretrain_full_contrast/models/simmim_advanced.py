...
class SimMIM(nn.Module):
    ...
    def forward(self, x, mask, x_org):
        ...
        if self.decoder == 'pixel_shuffle':
            ...
            x_rec = self.conv(self.transp_conv5(self.transp_conv4(self.transp_conv3(self.transp_conv2(self.transp_conv1(z))))))
        elif self.decoder == 'upsample':
            x_rec = (self.upsample(self.conv_block5(self.upsample(self.conv_block4(self.upsample(self.conv_block3(self.upsample(self.conv_block2(self.upsample(self.conv_block1(z)))))))))))
...
def build_simmim(args):
    ...
    if model_type == 'swin':
        ...
        encoder_stride = 32
        model = SimMIMSkip(encoder=encoder, encoder_stride=encoder_stride, loss=args.loss_type, img_size=(args.roi_x, args.roi_y, args.roi_z),
        ...
        encoder_stride = 32
        model = SimMIMSkip_light(encoder=encoder, encoder_stride=encoder_stride, loss=args.loss_type, img_size=(args.roi_x, args.roi_y, args.roi_z),
                          in_channels=args.in_channels,
                          out_channels=args.out_channels,
                          feature_size=args.feature_size,
                          drop_rate=0.0,
